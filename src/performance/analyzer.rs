...
impl StrategyAnalyzer {
    ...
    pub fn get_insights(&self) -> Option<HashMap<String, String>> {
        ...
        insights.insert("last_signal".to_string(), self.last_signal.clone().unwrap_or_else(|| "none".to_string()));
        ...
        // Calculate total PnL
        let total_pnl: f64 = self.trade_history.iter()
            .filter_map(|t| t.pnl)
            .sum();
    ...
    fn suggest_win_rate_improvement(
        ...
        let mut suggestion = OptimizationSuggestion {
            ...
            current_value: current_params.get("confirmation_period").copied().unwrap_or(1.0),
            suggested_value: 0.0,
            confidence: 0.7,
            reason: format!("Win rate ({:.1}%) is below minimum threshold ({:.1}%)",
    ...
    fn suggest_drawdown_reduction(
        ...
        OptimizationSuggestion {
            ...
            confidence: 0.8,
            reason: format!("Current drawdown ({:.1}%) exceeds maximum threshold ({:.1}%)",
        ...
        if metrics.total_trades >= 30 {
            ...
            if (current_pos_size - kelly).abs() > 0.05 {
                suggestions.push(OptimizationSuggestion {
                    ...
                    confidence: 0.6,
                    reason: format!("Kelly Criterion suggests optimal position size of {:.1}%", kelly * 100.0),
                });
            }
        }
...
mod tests {
    ...
    fn test_analyzer_suggestions() {
        ...
        assert!(suggestions.iter().any(|s| s.parameter.contains("confirmation_period") || 
