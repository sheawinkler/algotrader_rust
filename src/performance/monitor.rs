...
use crate::{
    ...
    utils::types::{MarketData, Order, OrderSide, Position},
    utils::Result,
};

use super::{analyzer::StrategyAnalyzer, metrics::StrategyMetrics};
...
impl PerformanceMonitor {
    ...
    pub async fn pause_strategy(&self, strategy_name: &str, reason: &str) -> Result<()> {
        ...
        self.add_alert(AlertType::StrategyPaused(reason.to_string()))
            .await;
    ...
    pub async fn resume_strategy(&self, strategy_name: &str) -> Result<()> {
        ...
        if paused.remove(strategy_name).is_some() {
            self.add_alert(AlertType::StrategyResumed("Manual resume".to_string()))
                .await;
    ...
    async fn check_performance(&self, strategy_name: &str) -> Result<()> {
        ...
        if !actions.is_empty() {
            ...
            if self.config.enable_circuit_breaker {
                ...
                self.add_alert(AlertType::CircuitBreakerTriggered(format!(
                    "Strategy {} paused: {}",
                ...
                // Just log a warning
                warn!("Performance issue detected for {}: {}", strategy_name, reason);
            }
        }
    ...
    pub async fn get_recommended_position_size(
        &self, strategy_name: &str, symbol: &str, account_balance: f64,
            ...
            if self.config.enable_adaptive_sizing {
                ...
                if let Some(market_data) = market_data {
                    ...
                    let volatility_adjustment =
                        1.0 / (1.0 + (volatility * self.config.volatility_factor));
                    position_size *= volatility_adjustment;
                }

                // Check correlation with other strategies
                let correlation_penalty = self
                    .calculate_correlation_penalty(strategy_name, symbol)
                    .await?;
        ...
        debug!(
            ...
            symbol,
            position_size,
            (position_size / account_balance * 100.0),
            account_balance
        );
    ...
    async fn calculate_volatility(&self, market_data: &MarketData) -> Result<f64> {
        ...
        let volatility = if avg_price > 0.0 {
    ...
    async fn calculate_correlation_penalty(
        ...
        let penalty = if max_correlation > self.config.correlation_threshold {
            ...
            0.5 * ((max_correlation - self.config.correlation_threshold)
    ...
    pub async fn get_performance_summary(&self) -> Result<String> {
        ...
        for (name, metric) in metrics.iter() {
            ...
            let last_signal = analyzer
                .and_then(|a| a.last_signal())
    ...
    pub async fn update_market_regime(&self, market_data: &MarketData) -> Result<()> {
        ...
        let price_change =
            (market_data.close - market_data.open.unwrap_or(0.0)) / market_data.open.unwrap_or(1.0);
        let volatility = (market_data.high.unwrap_or(0.0) - market_data.low.unwrap_or(0.0))
            / market_data.open.unwrap_or(1.0);
        ...
        if *current_regime != Some(new_regime) {
            ...
            *current_regime = Some(new_regime);
            ...
            self.add_alert(AlertType::MarketRegimeChanged(new_regime))
                .await;
            info!(
...
mod tests {
    ...
    async fn test_performance_monitor() {
        ...
        // Record some trades
        monitor
            .record_trade("TestStrategy", &order, None, 100.0, 0.0, None)
            .await
            .unwrap();
        monitor
            .record_trade("TestStrategy", &order, None, -50.0, 0.0, None)
            .await
            .unwrap();
