...
impl TrendFollowingStrategy {
    ...
    fn update_trend_direction(&mut self, price: f64) {
        ...
        let strong_uptrend = fast_ema > medium_ema 
            && medium_ema > slow_ema 
            && macd_line > macd_signal 
            && adx > 25.0;
            
        // Strong downtrend conditions
        let strong_downtrend = fast_ema < medium_ema 
            && medium_ema < slow_ema 
            && macd_line < macd_signal 
            && adx > 25.0;
            ...
            match self.trend_direction {
                TrendDirection::Uptrend => {
                    ...
                    if market_data.close > IndicatorValue::value(&self.fast_ema) 
                        && market_data.low.unwrap_or(market_data.close) <= IndicatorValue::value(&self.fast_ema) {
                ...
                TrendDirection::Downtrend => {
                    ...
                    if market_data.close < IndicatorValue::value(&self.fast_ema) 
                        && market_data.high.unwrap_or(market_data.close) >= IndicatorValue::value(&self.fast_ema) {
...
mod tests {
    ...
    async fn test_trend_following_strategy() {
        ...
        for i in 0..100 {
            ...
            if i == 60 { // Give indicators time to warm up
