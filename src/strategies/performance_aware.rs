...
use crate::trading::{MarketData, Signal, Order, Position};
...
impl<T: TradingStrategy + AdaptiveStrategy + Send + Sync + 'static> TradingStrategy for PerformanceAwareStrategy<T> {
    ...
    fn on_order_filled(&mut self, order: &Order) {
        ...
        tokio::spawn({
            ...
            async move {
                let _ = monitor.record_trade(&strategy_name, &order, None, pnl, 0.0, None).await;
            }
        });
...
mod tests {
    ...
    async fn test_performance_aware_strategy() {
        ...
        let inner = MockStrategy {
            name: "TestStrategy".to_string(),
            params: params.clone(),
        };
        
        let mut strategy = PerformanceAwareStrategy::new(
            inner,
            monitor,
            analyzer,
            params,
