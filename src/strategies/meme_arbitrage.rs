...
impl MemeArbitrageStrategy {
    pub fn new(
        symbol: &str, timeframe: TimeFrame, max_position_size: f64, max_slippage_pct: f64,
        max_consecutive_losses: u32,
    ) -> Self {
    ...
    fn calculate_arbitrage_score(
        &self, prices: &HashMap<String, f64>,
    ...
    fn update_performance_metrics(&mut self, trade: &TradeRecord) {
        ...
        if let (Some(pnl), Some(pnl_pct)) = (trade.pnl, trade.pnl_pct) {
            ...
            // Update win rate
            self.performance_metrics.win_rate = self.performance_metrics.winning_trades as f64
    ...
    fn review_and_adjust(&mut self) {
        ...
        if metrics.consecutive_losses >= self.max_consecutive_losses {
            ...
            info!(
                "Adjusted strategy: max_position_size={:.4}, DEX weights: {:?}",
