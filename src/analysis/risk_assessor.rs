...
impl RiskFactor {
    ...
    pub fn score_contribution(&self) -> f64 {
        match self {
            RiskFactor::HighDevHolding(pct) => {
                if *pct > 50.0 { 80.0 }
                else if *pct > 30.0 { 60.0 }
                else if *pct > 15.0 { 30.0 }
                else { 0.0 }
            },
            RiskFactor::HighInsiderHolding(pct) => {
                if *pct > 70.0 { 90.0 }
                else if *pct > 50.0 { 70.0 }
                else if *pct > 30.0 { 40.0 }
                else { 0.0 }
            },
            RiskFactor::LowLiquidity(sol) => {
                if *sol < 10_000.0 { 80.0 }
                else if *sol < 50_000.0 { 50.0 }
                else if *sol < 100_000.0 { 20.0 }
                else { 0.0 }
            },
            RiskFactor::NewToken(days) => {
                if *days < 1 { 70.0 }
                else if *days < 7 { 40.0 }
                else if *days < 30 { 20.0 }
                else { 0.0 }
            },
            RiskFactor::SuspiciousTokenAccount => 60.0,
            RiskFactor::HighVolatility(vol) => {
                if *vol > 1.0 { 50.0 }
                else if *vol > 0.5 { 30.0 }
                else { 0.0 }
            },
            RiskFactor::ConcentratedHolders(count) => {
                if *count <= 3 { 80.0 }
                else if *count <= 10 { 50.0 }
                else if *count <= 25 { 20.0 }
                else { 0.0 }
            },
            RiskFactor::RecentLargeTransfers(count) => {
                if *count > 10 { 70.0 }
                else if *count > 5 { 40.0 }
            ...
            RiskFactor::HighSellPressure(pct) => {
                if *pct > 70.0 { 80.0 }
                else if *pct > 50.0 { 50.0 }
                else if *pct > 30.0 { 20.0 }
                else { 0.0 }
            },
            RiskFactor::LowHolderCount(count) => {
                if *count < 100 { 60.0 }
                else if *count < 500 { 30.0 }
                else { 0.0 }
            },
    ...
    pub fn description(&self) -> String {
        match self {
            ...
            RiskFactor::HighInsiderHolding(pct) => format!("High insider holding: {:.1}% of supply", pct),
            ...
            RiskFactor::ConcentratedHolders(count) => format!("Concentrated holders: {} control >50%", count),
            RiskFactor::RecentLargeTransfers(count) => format!("{} large transfers in 24h", count),
            RiskFactor::UnverifiedProgram => "Unverified token program".to_string(),
            RiskFactor::NoLiquidityLock => "No locked liquidity".to_string(),
            RiskFactor::HighSellPressure(pct) => format!("High sell pressure: {:.1}% of volume", pct),
...
impl RiskAssessor {
    ...
    pub async fn assess_token(&mut self, token_address: &str) -> Result<RiskAssessment> {
        ...
        self.cached_assessments.insert(
            token_address.to_string(), 
            (assessment.clone(), Utc::now())
        );
    ...
    pub fn is_acceptable_risk(&self, assessment: &RiskAssessment) -> bool {
        assessment.risk_score <= self.config.max_risk_score &&
