                ...
                if let Ok(json) = serde_json::from_str::<Value>(&txt) {
                    ...
                    if json.is_array() && json.as_array().unwrap().len() > 3 {
                        ...
                        if let Some(trades) = arr.get(1).and_then(|v| v.as_array()) {
                            let symbol = arr.get(3).and_then(|v| v.as_str()).unwrap_or("").to_string();
                            for trade in trades {
                                if let Some(trade_arr) = trade.as_array() {
                                    let price = trade_arr.get(0).and_then(|v| v.as_str()).and_then(|s| s.parse().ok()).unwrap_or(0.0);
                                    let qty = trade_arr.get(1).and_then(|v| v.as_str()).and_then(|s| s.parse().ok()).unwrap_or(0.0);
                                    let side = trade_arr.get(3).and_then(|v| v.as_str()).unwrap_or("").to_string();
                                    let timestamp = trade_arr.get(2).and_then(|v| v.as_f64()).map(|f| (f * 1000.0) as i64).unwrap_or(0);
