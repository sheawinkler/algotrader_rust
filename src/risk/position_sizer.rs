...
impl PositionSizer for FixedFractionalSizer {
    async fn size(&self, equity: f64, _symbol: &str) -> f64 { equity * self.pct }
    fn box_clone(&self) -> Box<dyn PositionSizer> { Box::new(self.clone()) }
}
...
impl KellySizer {
    pub fn new(win_rate: f64, payoff_ratio: f64, cap: f64) -> Self { Self { win_rate, payoff_ratio, cap } }
}
...
impl PositionSizer for KellySizer {
    async fn size(&self, equity: f64, _symbol: &str) -> f64 {
        ...
        let b = if self.payoff_ratio <= 0.0 { 1.0 } else { self.payoff_ratio };
        let q = 1.0 - p;
        let kelly = ((b * p) - q) / b;
...
impl PositionSizer for LiveKellySizer {
    async fn size(&self, equity: f64, _symbol: &str) -> f64 {
        ...
        let avg_win = metrics.iter().filter(|m| m.winning_trades>0).map(|m| m.total_pnl.max(0.0)).sum::<f64>() / (total_wins.max(1) as f64);
        let avg_loss = metrics.iter().filter(|m| m.losing_trades>0).map(|m| m.total_pnl.min(0.0).abs()).sum::<f64>() / (total_losses.max(1) as f64);
